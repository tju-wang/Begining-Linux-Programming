...
int main() {
    ...
    for (i = 0; i < ITEMS_USED; i++) {
        ...
        key_datum.dptr = (void *)key_to_use;    //以数据的前20字节作为本数据块的索引  将索引数据和大小存入datum结构体中
        key_datum.dsize = strlen(key_to_use);
        data_datum.dptr = (void *)&items_to_store[i];   //创建数据结构体  
        data_datum.dsize = sizeof(struct test_data);

        result = dbm_store(dbm_ptr, key_datum, data_datum, DBM_REPLACE);//存入数据  若存在相同索引  将老数据覆盖
        if (result != 0) {
    ...
    data_datum = dbm_fetch(dbm_ptr, key_datum); //查找数据
    if (data_datum.dptr) {
